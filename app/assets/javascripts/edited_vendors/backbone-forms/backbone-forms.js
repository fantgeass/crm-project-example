...
var Form = (function() {
  ...
  return Backbone.View.extend({
    ...
    initialize: function(options) {
      ...
      if (this.model && this.options.serverSideErrors) {
        this.model.on('error', this.serverSideErrors.bind(this));
      }
    ...
    /**
    * Handle server side error & render it in form
    * @param model
    * @param collection
    * @returns {Array}
    */
    serverSideErrors: function(model, collection) {
      var base, control_group, errors, field, form, id, _results;
      errors = jQuery.parseJSON(collection.responseText).errors;
      form = this.$el;
      ...
      for (field in errors) {
          ...
          if (control_group.length > 0) {
              _results.push(control_group.addClass('error').find('.help-block').html(errors[field].join(', ')));
          } else if (field === 'base') {
              _results.push(base.html(base.html() + '<p>' + errors[field].join('<br>') + '</p>'));
          } else {
              _results.push(base.html(base.html() + '<p>' + field + ': ' + errors[field].join(', ') + '</p>'));
          }
      }
      return _results;
    },
